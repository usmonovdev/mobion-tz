...
const Sort = ({ data, selected, setSelected }) => {
  ...
  return (
    <div className="phone:w-[200px] w-full z-[1000]" data-aos="fade-up">
      <Listbox value={selected} onChange={setSelected}>
        <div className="relative mt-1">
          <Box
            bgcolor={"custom.contact"}
            className="rounded-lg border border-[#02C981]"
          >
          ...
          >
            <Listbox.Options
              className={`absolute mt-1 max-h-60 w-full overflow-auto rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm ${
                palette.mode == "light" ? "bg-[#F6F7F9]" : "bg-[#2C2C2C]"
              }`}
            >
